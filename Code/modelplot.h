...
void* ThreadSpinner(void* t){
    ...
    string filename = "model_b"+ratestr+"_N"+LATTICE_SIZEstr+"_gens"+gen_str+".vtk";
    plot.open(filename.c_str());
    for(int i = 0; i < WORKLOAD; i++){
        while(!CellEvolution(obj,i)){
            printf("total death... starting over.");
            Reinitialize(obj);
        }
        //Status(obj->tree);
    }
    RecordPosData(*(obj->tree),plot,obj->NUMBER_OF_GENERATIONS);
    
    plot.close();

    printf("File: %s in local directory\n",filename.c_str()); fflush(stdout);
    return (void*) obj;
}
...
bool CellEvolution(thread_obj* obj,int index){
    ...
    while(obj->NUMBER_OF_GENERATIONS < MAX_GENERATIONS){
        ...
        if(!(obj->NUMBER_OF_GENERATIONS%100)){ printf("%d\n",obj->NUMBER_OF_GENERATIONS);}
        // Exit conditions
...
void UpdatePos(thread_obj* obj){
    ...
    for(obj->sit=obj->active->begin();obj->sit!=obj->active->end();obj->sit++){
        ...
        if(!Selective_color){
            tip.color = (float) obj->NUMBER_OF_GENERATIONS / MAX_GENERATIONS;
        }else if(!b->fixation){
