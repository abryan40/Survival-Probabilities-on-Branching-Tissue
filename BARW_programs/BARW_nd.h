 ...
 * Collects survival statistics for various sets of parameters hardcoded into globals.
 * Does not count data for structures that die entirely (total death).
 */
...
std::uniform_real_distribution<double> d2ddist(0.0,(1 - cos(2 * (M_PI/double(50))))/2);
...
void* ThreadSpinner(void* t){
    ...
    for(i = 0; i < WORKLOAD; i++){
        ...
        if(obj->Total_death){
            ...
            obj->Mutant_pop_percent[i] = RedPercent(obj);
        }
    ...
    obj->attempts = i;
    obj->AVERAGE_RED_PERCENT = avg(obj->Mutant_pop_percent,obj->attempts);
    return (void*) obj;
}
...
bool CellEvolution(thread_obj* obj,int index){
    ...
    while(obj->NUMBER_OF_GENERATIONS < MAX_GENERATIONS){
        ...
        // Loop to check for expansion of N_0 (lattice_size)
                ...
                for(obj->sit=obj->active->begin();obj->sit!=obj->active->end();obj->sit++){
                    ...
                    if(b->growing && !(b->termination)){
                        if(b->lattice_size >= 2*LATTICE_SIZE){
                            ...
                            // Not optimal but keeps things safe.
...
bool AreTheyAllFixated(thread_obj* obj){
    if(obj->active->size() == 0){ return false; }
    branch* b;
    for(obj->sit=obj->active->begin();obj->sit!=obj->active->end();obj->sit++){
        b = *(obj->sit);
        if(!b->fixation){
...
bool AreTheyAllTerminated(thread_obj* obj){
    if(obj->active->size() == 0){ return true; }
    obj->sit = obj->active->begin();
    branch* b;
...
void BranchProcess(thread_obj* obj,branch* b,int i){
    ...
    int rand = floor(branchdist(mt)*LATTICE_SIZE);
    b->walker1 += rand;
    b->walker2 += rand;
